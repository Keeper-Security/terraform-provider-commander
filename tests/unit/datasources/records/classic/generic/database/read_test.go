...
import (
	...
	databaseds "github.com/Keeper-Security/terraform-provider-commander/internal/provider/datasources/records/classic/generic/database"
...
func getDSSchema(t *testing.T) dschema.Schema {
	t.Helper()
	d := databaseds.NewDatabaseDataSource().(*databaseds.DatabaseDataSource)
	var resp datasource.SchemaResponse
	d.Schema(context.Background(), datasource.SchemaRequest{}, &resp)
	return resp.Schema
}

func newConfiguredDataSource(t *testing.T, server *httptest.Server) *databaseds.DatabaseDataSource {
	t.Helper()
	am := &api.ApiManager{
...
func dsVaultRecordJSON(uid, title, login, password, hostname, port, dbType string) interface{} {
	return map[string]interface{}{
		...
		"fields": []map[string]interface{}{
			{"type": "text", "label": "type", "value": []string{dbType}},
...
func newDSConfigRaw(t *testing.T, sch dschema.Schema, databaseLookup string) tftypes.Value {
	t.Helper()
	...
	for name, attrType := range objType.AttributeTypes {
		if name == "database" {
			vals[name] = tftypes.NewValue(tftypes.String, databaseLookup)
		} else {
...
func TestRead_DS_Success(t *testing.T) {
	...
	server := helpers.StartCommandServer(mock, func(cmd string, _ int) (string, interface{}) {
		if strings.Contains(cmd, "get") && strings.Contains(cmd, "--format json") {
			return "ok", dsVaultRecordJSON("uid-db-1", "Test DB", "root", "secret", "127.0.0.1", "8000", "SQL")
		}
		return "ok", nil
	})
	defer server.Close()
	...
	configRaw := newDSConfigRaw(t, sch, "uid-db-1")
...
func TestRead_DS_EmptyLookup(t *testing.T) {
	...
	d.Read(context.Background(), req, &resp)
	if !resp.Diagnostics.HasError() {
		t.Error("expected diagnostics when database is empty")
	}
}
...
func TestRead_DS_WrongRecordType(t *testing.T) {
	...
	d.Read(context.Background(), req, &resp)
	if !resp.Diagnostics.HasError() {
		t.Error("expected diagnostics when record type is not databaseCredentials")
	}
}
