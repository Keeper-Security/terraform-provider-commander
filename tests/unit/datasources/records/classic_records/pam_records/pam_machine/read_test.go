...
import (
	...
	pammachine "github.com/Keeper-Security/terraform-provider-commander/internal/provider/datasources/records/classic_records/pam_records/pam_machine"
...
func newConfiguredDataSource(t *testing.T, server *httptest.Server) *pammachine.PamMachineDataSource {
	t.Helper()
	am := &api.ApiManager{
...
func TestRead_DS_Success(t *testing.T) {
	...
	responseForCommand := func(cmd string, idx int) (string, interface{}) {
		if strings.Contains(cmd, "get") && strings.Contains(cmd, "--format json") {
			return "ok", dsVaultRecordJSON("uid-abc", "My Machine", "notes", "10.0.0.1", "22")
		}
		return "ok", nil
	}
...
func newDSConfigRaw(t *testing.T, sch dschema.Schema, pamMachine string) tftypes.Value {
	t.Helper()
	...
	for name, attrType := range objType.AttributeTypes {
		if name == "pam_machine" {
			vals[name] = tftypes.NewValue(tftypes.String, pamMachine)
		} else {
