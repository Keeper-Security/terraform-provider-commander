...
import (
	...
	pamdirectory "github.com/Keeper-Security/terraform-provider-commander/internal/provider/datasources/pam_records/pam_directory"
...
func newConfiguredDataSource(t *testing.T, server *httptest.Server) *pamdirectory.PamDirectoryDataSource {
	t.Helper()
	am := &api.ApiManager{
...
func TestRead_DS_Success(t *testing.T) {
	...
	responseForCommand := func(cmd string, idx int) (string, interface{}) {
		if strings.Contains(cmd, "get") && strings.Contains(cmd, "--format json") {
			return "ok", dsVaultRecordJSON("uid-abc", "My Directory", "notes", "10.0.0.1", "636")
		}
		return "ok", nil
	}
...
func newDSConfigRaw(t *testing.T, sch dschema.Schema, pamDirectory string) tftypes.Value {
	t.Helper()
	...
	for name, attrType := range objType.AttributeTypes {
		if name == "pam_directory" {
			vals[name] = tftypes.NewValue(tftypes.String, pamDirectory)
		} else {
