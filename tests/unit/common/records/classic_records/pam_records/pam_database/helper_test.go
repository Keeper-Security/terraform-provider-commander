...
func TestMapVaultRecordGetResponse_BasicFields(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Fields: []utils.VaultRecordFieldResponse{
			{Type: "pamHostname", Value: json.RawMessage(`[{"hostName":"db.example.com","port":"5432"}]`)},
			{Type: "checkbox", Label: "useSSL", Value: json.RawMessage(`[true]`)},
			{Type: "text", Label: "databaseId", Value: json.RawMessage(`["db-id-123"]`)},
			{Type: "databaseType", Value: json.RawMessage(`["mongodb"]`)},
	...
	diags := commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if diags.HasError() {
	...
	if state.Title.ValueString() != "My Database" {
		t.Errorf("expected title My Database, got %s", state.Title.ValueString())
	}
	...
	if state.DatabaseId.ValueString() != "db-id-123" {
		t.Errorf("expected database_id db-id-123, got %s", state.DatabaseId.ValueString())
	}
	if state.DatabaseType.ValueString() != "mongodb" {
		t.Errorf("expected database_type mongodb, got %s", state.DatabaseType.ValueString())
	}
...
func TestMapVaultRecordGetResponse_HostnameOrIP(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "Host Test",
		Fields: []utils.VaultRecordFieldResponse{
			{Type: "pamHostname", Value: json.RawMessage(`[{"hostName":"10.0.0.1","port":"3306"}]`)},
		},
	}
	...
	if state.HostnameOrIP.AdministrativePort.ValueInt32() != 3306 {
		t.Errorf("expected port 3306, got %d", state.HostnameOrIP.AdministrativePort.ValueInt32())
	}
}
...
func TestMapVaultRecordGetResponse_HostnameWithoutPort(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "No Port",
		Fields: []utils.VaultRecordFieldResponse{
			{Type: "pamHostname", Value: json.RawMessage(`[{"hostName":"db.test.com","port":""}]`)},
		},
	}
...
func TestMapVaultRecordGetResponse_InvalidPortString(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "Bad Port",
		Fields: []utils.VaultRecordFieldResponse{
			{Type: "pamHostname", Value: json.RawMessage(`[{"hostName":"host","port":"notanumber"}]`)},
		},
	}
...
func TestMapVaultRecordGetResponse_BadHostnameJSON(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "Bad JSON",
		Fields: []utils.VaultRecordFieldResponse{
			{Type: "pamHostname", Value: json.RawMessage(`{invalid`)},
		},
	}
...
func TestExtractPamHostnameFieldValue_Success(t *testing.T) {
	...
	model := commonpamdatabase.ExtractPamHostnameFieldValue(fields)
	if model == nil {
...
func TestExtractCheckboxFieldValue_True(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "SSL True",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "checkbox", Label: "useSSL", Value: json.RawMessage(`[true]`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if !state.UseSSL.ValueBool() {
...
func TestExtractCheckboxFieldValue_False(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "SSL False",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "checkbox", Label: "useSSL", Value: json.RawMessage(`[false]`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if state.UseSSL.ValueBool() {
...
func TestExtractCheckboxFieldValue_BadJSON(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "SSL Bad",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "checkbox", Label: "useSSL", Value: json.RawMessage(`{invalid`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
...
func TestExtractCheckboxFieldValue_EmptyArray(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "SSL Empty",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "checkbox", Label: "useSSL", Value: json.RawMessage(`[]`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if !state.UseSSL.IsNull() {
...
func TestExtractCheckboxFieldValue_NoMatch(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "No SSL",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "checkbox", Label: "otherCheckbox", Value: json.RawMessage(`[true]`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if !state.UseSSL.IsNull() {
...
func TestExtractDatabaseTypeFieldValue_MongoDB(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		RecordUID: "uid-dt",
		Title:     "DT",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "databaseType", Value: json.RawMessage(`["mongodb"]`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if state.DatabaseType.ValueString() != "mongodb" {
		t.Errorf("expected mongodb, got %s", state.DatabaseType.ValueString())
	}
}

func TestExtractDatabaseTypeFieldValue_PostgreSQL(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		RecordUID: "uid-dt-pg",
		Title:     "DT PG",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "databaseType", Value: json.RawMessage(`["postgresql"]`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if state.DatabaseType.ValueString() != "postgresql" {
		t.Errorf("expected postgresql, got %s", state.DatabaseType.ValueString())
	}
}

func TestExtractDatabaseTypeFieldValue_BadJSON(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		RecordUID: "uid-dt-bad",
		Title:     "DT Bad",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "databaseType", Value: json.RawMessage(`{invalid`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if !state.DatabaseType.IsNull() {
		t.Error("expected null database_type for bad JSON")
	}
}

func TestExtractDatabaseTypeFieldValue_EmptyArray(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		RecordUID: "uid-dt-empty",
		Title:     "DT Empty",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "databaseType", Value: json.RawMessage(`[]`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if !state.DatabaseType.IsNull() {
		t.Error("expected null database_type for empty array")
	}
}

func TestExtractDatabaseTypeFieldValue_EmptyString(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		RecordUID: "uid-dt-es",
		Title:     "DT EmptyStr",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "databaseType", Value: json.RawMessage(`[""]`)}},
	...
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if !state.DatabaseType.IsNull() {
		t.Error("expected null database_type for empty string value")
	}
}

func TestExtractDatabaseTypeFieldValue_NoField(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		RecordUID: "uid-dt-nf",
		Title:     "DT NoField",
		Fields:    []utils.VaultRecordFieldResponse{{Type: "text", Label: "databaseId", Value: json.RawMessage(`["id"]`)}},
	}
	var state commonpamdatabase.PamDatabaseResourceModel
	commonpamdatabase.MapVaultRecordGetResponseToPamDatabaseModel(rec, &state)
	if !state.DatabaseType.IsNull() {
		t.Error("expected null database_type when no databaseType field")
	}
}
...
func TestMapVaultRecordGetResponse_WithPamSettings(t *testing.T) {
	rec := &utils.VaultRecordGetResponse{
		...
		Title:     "With Settings",
		Fields: []utils.VaultRecordFieldResponse{
			{Type: "pamSettings", Value: json.RawMessage(`[{"allowSupplyHost":true,"portForward":{"port":"27017"},"connection":{"protocol":"kubernetes","port":"8443"}}]`)},
		},
		...
		DagDebug: &utils.DagDebugResponse{
			AllEdges: []utils.DagDebugEdgeResponse{{Type: "link", HeadUID: "config-uid-456"}},
		},
