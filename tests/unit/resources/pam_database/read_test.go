...
func TestRead_Success(t *testing.T) {
	...
	responseForCommand := func(cmd string, idx int) (string, interface{}) {
		if strings.Contains(cmd, "get") && strings.Contains(cmd, "--format json") {
			return "ok", vaultRecordGetJSON("uid-abc", "My Database", "notes", "10.0.0.1", "5432")
		}
		return "ok", nil
	}
	...
	hostVals := newHostnameValues("10.0.0.1", float64(5432))
	rawState := tftypes.NewValue(objType, newPlanStateValues(
		"uid-abc", "My Database", hostVals, nil, nil, nil, nil, nil, nil, nil,
	))
...
func TestRead_NoApiManager(t *testing.T) {
	...
	rawState := tftypes.NewValue(objType, newPlanStateValues(
		"uid-abc", "Title", hostVals, nil, nil, nil, nil, nil, nil, nil,
	))
...
func TestRead_ResourceNotFound_RemovesFromState(t *testing.T) {
	...
	hostVals := newHostnameValues("host.com", nil)
	rawState := tftypes.NewValue(objType, newPlanStateValues(
		"uid-missing", "Title", hostVals, nil, nil, nil, nil, nil, nil, nil,
	))
...
func TestRead_NilResponseData(t *testing.T) {
	mock := &helpers.CommandServer{}
	responseForCommand := func(cmd string, idx int) (string, interface{}) {
		return "ok", nil
	}
	...
	r.Read(context.Background(), req, &resp)
	if resp.Diagnostics.HasError() {
		t.Errorf("expected no error for nil response (resource removed from state): %v", resp.Diagnostics)
	}
}
