...
func TestRead_Success(t *testing.T) {
	...
	responseForCommand := func(cmd string, idx int) (string, interface{}) {
		if strings.Contains(cmd, "get") && strings.Contains(cmd, "--format json") {
			return "ok", vaultRecordGetJSON("uid-abc", "My Machine", "notes", "10.0.0.1", "22")
		}
		return "ok", nil
	}
	...
	hostVals := newHostnameValues("10.0.0.1", float64(22))
	rawState := tftypes.NewValue(objType, newPlanStateValues(
		"uid-abc", "My Machine", hostVals, nil, nil, nil, nil, nil, nil, nil,
	))
