...
func TestEnterpriseNodeResource_Create_WithParentAndManagedCompanySame(t *testing.T) {
	mock := &helpers.CommandServer{}
	responseForCommand := func(cmd string, idx int) (string, interface{}) {
		if strings.Contains(cmd, "enterprise-node --add") && strings.Contains(cmd, "--parent 'root'") {
	...
	r := newConfiguredResource(t, server)
	sch, objType := getSchema(t)
	// Parent same as managed company -> code uses "root" in command
...
func TestEnterpriseNodeResource_Create_WithToggleIsolated(t *testing.T) {
	mock := &helpers.CommandServer{}
	responseForCommand := func(cmd string, idx int) (string, interface{}) {
		if strings.Contains(cmd, "enterprise-node --add") && strings.Contains(cmd, "--toggle-isolated") {
			return "Node ID: 789", nil
		}
		return "ok", nil
	}
	server := startMockServer(mock, responseForCommand)
