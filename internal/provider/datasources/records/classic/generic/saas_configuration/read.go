...
func (d *SaasConfigurationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data SaasConfigurationDataSourceModel
	...
	lookup := strings.TrimSpace(data.SaasConfiguration.ValueString())
	if lookup == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "SaaS configuration record identifier is empty")
		return
	}
	...
	if apiResp == nil || apiResp.Data == nil {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, fmt.Sprintf("SaaS configuration record '%s' not found", lookup))
		return
	}
	...
	resp.Diagnostics.Append(commonrecordsaasconfiguration.MapVaultRecordGetResponseToSaasConfigurationModel(&rec, data.FolderLocation, &data.SaasConfigurationModel)...)
	if resp.Diagnostics.HasError() {
		return
	}

	if err := classic_share.MapResponseToModel(rec.UserPermissions, &data.ShareModel); err != nil {
		resp.Diagnostics.AddError(ErrSummaryReadSaasConfigurationDataSource, err.Error())
		return
	}
