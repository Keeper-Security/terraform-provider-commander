...
func (d *HealthInsuranceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data HealthInsuranceDataSourceModel
	...
	lookup := strings.TrimSpace(data.HealthInsurance.ValueString())
	if lookup == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "Health Insurance record identifier is empty")
		return
	}
	...
	if apiResp == nil || apiResp.Data == nil {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, fmt.Sprintf("Health Insurance record '%s' not found", lookup))
		return
	}
	...
	resp.Diagnostics.Append(commonrecordhealthinsurance.MapVaultRecordGetResponseToHealthInsuranceModel(&rec, data.FolderLocation, &data.HealthInsuranceModel)...)
	if resp.Diagnostics.HasError() {
		return
	}

	if err := new_share.MapResponseToModel(rec.UserPermissions, &data.ShareModel); err != nil {
		resp.Diagnostics.AddError(ErrSummaryReadHealthInsuranceDataSource, err.Error())
		return
	}
