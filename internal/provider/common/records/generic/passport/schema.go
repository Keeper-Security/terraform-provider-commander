...
// SharedAttributes returns the Passport resource attribute map shared between
...
func SharedAttributes() map[string]schema.Attribute {
	return utils.MergeResourceAttributes(
		...
		map[string]schema.Attribute{
			...
			"date_issued":     commonrecordsutils.OptionalDateStringField("Date issued"),
...
// SharedDataSourceAttributes returns computed Passport data source attributes
// shared between classic and new data sources. Callers add the lookup key (e.g.
// passport) and share-extension attributes separately.
func SharedDataSourceAttributes() map[string]dschema.Attribute {
	return utils.MergeDataSourceAttributes(
		...
		map[string]dschema.Attribute{
			...
			"date_issued": commonrecordsutils.ComputedDateStringField(
				"Date the passport was issued (YYYY-MM-DD).",
				"Date the passport was **issued** (`YYYY-MM-DD`).",
			),
