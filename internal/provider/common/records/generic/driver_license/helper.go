...
// BuildAddCommand builds a record-add command for a driverLicense record.
func BuildAddCommand(cmd string, data DriverLicenseModel) string {
	var extra []string
	...
	return commonrecordsutils.BuildRecordAdd(cmd, commonrecordsutils.RecordTypeDriverLicense, data.Title.ValueString(), data.FolderLocation, extra, custom, data.Notes)
}

// UpdateHasMutations reports whether plan differs from state on updatable driver's license fields.
func UpdateHasMutations(plan, state DriverLicenseModel) bool {
...
// BuildUpdateCommand builds a record-update command for changed driver's license fields.
func BuildUpdateCommand(cmd string, recordUID string, plan, state DriverLicenseModel) string {
	var extra []string
...
// MapVaultRecordGetResponseToDriverLicenseModel fills state from a `get <uid> --format json` payload.
func MapVaultRecordGetResponseToDriverLicenseModel(rec *utils.VaultRecordGetResponse, stateFolder types.String, m *DriverLicenseModel) diag.Diagnostics {
