...
func (r *PamDirectoryResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if recordUID == "" {
		resp.Diagnostics.AddError(ErrSummaryPamDirectoryRecordUpdateFailed, "new PAM directory record id is empty")
		return
	}
	...
	if commonpamdirectory.RecordUpdateHasMutations(plan.PamDirectoryResourceModel, state.PamDirectoryResourceModel) {
		cmd := commonpamdirectory.BuildUpdateCommand(utils.CmdNsfRecordUpdate, recordUID, plan.PamDirectoryResourceModel, state.PamDirectoryResourceModel)
	...
	if plan.PamSettings != nil {
		if err := commonpamrecords.ApplyMachineDirectoryPamSettings(ctx, r.ApiManager, utils.CmdNsfRecordUpdate, recordUID, plan.PamSettings, state.PamSettings); err != nil {
