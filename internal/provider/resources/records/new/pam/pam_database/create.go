...
func (r *PamDatabaseResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	command := commonpamdatabase.BuildAddCommand(utils.CmdNsfRecordAdd, data.PamDatabaseResourceModel)
	...
	if createdRecordUID == "" {
		resp.Diagnostics.AddError(ErrSummaryAddPamDatabaseRecordFailed, fmt.Sprintf("Failed to extract record UID from response. API response: %v", apiResp))
		return
	}
	data.Id = types.StringValue(createdRecordUID)

	if data.PamSettings != nil {
		if err := commonpamrecords.ApplyDatabasePamSettings(ctx, r.ApiManager, utils.CmdNsfRecordUpdate, createdRecordUID, data.PamSettings, nil); err != nil {
