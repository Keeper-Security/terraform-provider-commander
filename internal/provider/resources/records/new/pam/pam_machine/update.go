...
func (r *PamMachineResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if recordUID == "" {
		resp.Diagnostics.AddError(ErrSummaryPamMachineRecordUpdateFailed, "new PAM machine record id is empty")
		return
	}
	...
	if commonpammachine.RecordUpdateHasMutations(plan.PamMachineResourceModel, state.PamMachineResourceModel) {
		cmd := commonpammachine.BuildUpdateCommand(utils.CmdNsfRecordUpdate, recordUID, plan.PamMachineResourceModel, state.PamMachineResourceModel)
