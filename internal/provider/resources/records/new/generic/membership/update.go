...
func (r *MembershipResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if uid == "" {
		resp.Diagnostics.AddError(ErrSummaryUpdateFailed, "Membership record id is empty")
		return
	}
	...
	if commonrecordmembership.UpdateHasMutations(plan.MembershipModel, state.MembershipModel) {
		cmd := commonrecordmembership.BuildUpdateCommand(utils.CmdNsfRecordUpdate, uid, plan.MembershipModel, state.MembershipModel)
