...
func (r *MembershipResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if id == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "Membership record id is empty")
		return
	}
	...
	resp.Diagnostics.Append(commonrecordmembership.MapVaultRecordGetResponseToMembershipModel(&rec, state.FolderLocation, &state.MembershipModel)...)
	if resp.Diagnostics.HasError() {
		return
	}
