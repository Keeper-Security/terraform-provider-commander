...
func (r *PassportResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	if id == "" {
		resp.Diagnostics.AddError(utils.ERR_MSG_INVALID_IMPORT_ID, "Import ID cannot be empty. Use the Passport record UID.")
		return
	}
	m := PassportResourceModel{
		PassportModel: commonrecordpassport.PassportModel{
