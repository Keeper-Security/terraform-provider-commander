...
func (r *LoginResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var plan, state LoginResourceModel
	...
	if uid == "" {
		resp.Diagnostics.AddError(ErrSummaryUpdateFailed, "Login record id is empty")
		return
	}
	...
	if commonrecordlogin.UpdateHasMutations(plan.LoginModel, state.LoginModel) {
		cmd := commonrecordlogin.BuildUpdateCommand(utils.CmdNsfRecordUpdate, uid, plan.LoginModel, state.LoginModel)
