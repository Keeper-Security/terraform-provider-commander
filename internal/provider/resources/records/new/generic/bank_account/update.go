...
func (r *BankAccountResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var plan, state BankAccountResourceModel
	...
	if uid == "" {
		resp.Diagnostics.AddError(ErrSummaryUpdateFailed, "Bank Account record id is empty")
		return
	}
	...
	if commonrecordbankaccount.UpdateHasMutations(plan.BankAccountModel, state.BankAccountModel) {
		cmd := commonrecordbankaccount.BuildUpdateCommand(utils.CmdNsfRecordUpdate, uid, plan.BankAccountModel, state.BankAccountModel)
