...
func (r *ServerResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if id == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "Server record id is empty")
		return
	}
	...
	resp.Diagnostics.Append(commonrecordserver.MapVaultRecordGetResponseToServerModel(&rec, state.FolderLocation, &state.ServerModel)...)
	if resp.Diagnostics.HasError() {
		return
	}
