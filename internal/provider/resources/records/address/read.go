...
func (r *AddressResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if id == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "Address record id is empty")
		return
	}
	...
	if rec.Type != "" && rec.Type != records.RecordTypeAddress {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, fmt.Sprintf("vault record type is %q, expected %q", rec.Type, records.RecordTypeAddress))
		return
	}
