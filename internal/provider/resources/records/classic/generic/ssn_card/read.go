...
func (r *SsnCardResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	var state SsnCardResourceModel
	...
	if id == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "SSN Card record id is empty")
		return
	}
	...
	resp.Diagnostics.Append(commonrecordssncard.MapVaultRecordGetResponseToSsnCardModel(&rec, state.FolderLocation, &state.SsnCardModel)...)
	if resp.Diagnostics.HasError() {
		return
	}
