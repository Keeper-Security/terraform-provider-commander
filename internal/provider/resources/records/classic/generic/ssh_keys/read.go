...
func (r *SshKeysResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if id == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "SSH keys record id is empty")
		return
	}
	...
	commonrecordsshkeys.MapVaultRecordGetResponseToSshKeysModel(&rec, state.FolderLocation, &state.SshKeysModel)
