...
func (r *SecureNoteResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	var state SecureNoteResourceModel
	...
	if id == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "Secure note record id is empty")
		return
	}
	...
	resp.Diagnostics.Append(commonrecordsecurenote.MapVaultRecordGetResponseToSecureNoteModel(&rec, state.FolderLocation, &state.SecureNoteModel)...)
	if resp.Diagnostics.HasError() {
		return
	}
