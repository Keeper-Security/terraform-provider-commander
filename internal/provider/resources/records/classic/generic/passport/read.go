...
func (r *PassportResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if id == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "Passport record id is empty")
		return
	}
	...
	resp.Diagnostics.Append(commonrecordpassport.MapVaultRecordGetResponseToPassportModel(&rec, state.FolderLocation, &state.PassportModel)...)
	if resp.Diagnostics.HasError() {
		return
	}
