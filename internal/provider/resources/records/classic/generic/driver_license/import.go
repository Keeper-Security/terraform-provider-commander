...
func (r *DriverLicenseResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	id := strings.TrimSpace(req.ID)
	if id == "" {
		resp.Diagnostics.AddError(utils.ERR_MSG_INVALID_IMPORT_ID, "Import ID cannot be empty. Use the Driver's License record UID.")
		return
	}
	m := DriverLicenseResourceModel{
		DriverLicenseModel: commonrecorddriverlicense.DriverLicenseModel{
