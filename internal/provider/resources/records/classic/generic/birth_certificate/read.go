...
func (r *BirthCertificateResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if id == "" {
		resp.Diagnostics.AddError(ErrSummaryReadFailed, "Birth Certificate record id is empty")
		return
	}
	...
	resp.Diagnostics.Append(commonrecordbirthcertificate.MapVaultRecordGetResponseToBirthCertificateModel(&rec, state.FolderLocation, &state.BirthCertificateModel)...)
	if resp.Diagnostics.HasError() {
		return
	}
