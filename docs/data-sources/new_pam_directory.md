---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "commander_new_pam_directory Data Source - commander"
subcategory: ""
description: |-
  Use this data source to look up a new (nested-shared) PAM directory record by UID or name and read its per-user share permissions.
---

# commander_new_pam_directory (Data Source)

Use this data source to look up a **new (nested-shared) PAM directory** record by **UID** or **name** and read its **per-user share permissions**.

## Example Usage

```terraform
# ------------------------------------------------------------------
# Example 1 - Look up a new (nested-shared) PAM directory by UID or name
# ------------------------------------------------------------------
data "commander_new_pam_directory" "existing" {
  pam_directory = "_REPLACE_WITH_RECORD_UID_OR_NAME_"
}

output "pam_directory_id" {
  description = "Record UID from the data source read."
  value       = data.commander_new_pam_directory.existing.id
}

output "pam_directory_title" {
  value = data.commander_new_pam_directory.existing.title
}

output "pam_directory_hostname" {
  value = data.commander_new_pam_directory.existing.hostname_or_ip
}

output "pam_directory_use_ssl" {
  value = data.commander_new_pam_directory.existing.use_ssl
}

output "pam_directory_domain_name" {
  value = data.commander_new_pam_directory.existing.domain_name
}

output "pam_directory_alternative_ips" {
  value = data.commander_new_pam_directory.existing.alternative_ips
}

output "pam_directory_directory_id" {
  value = data.commander_new_pam_directory.existing.directory_id
}

output "pam_directory_directory_type" {
  value = data.commander_new_pam_directory.existing.directory_type
}

output "pam_directory_user_match" {
  value = data.commander_new_pam_directory.existing.user_match
}

output "pam_directory_provider_group" {
  value = data.commander_new_pam_directory.existing.provider_group
}

output "pam_directory_provider_region" {
  value = data.commander_new_pam_directory.existing.provider_region
}

output "pam_directory_notes" {
  value = data.commander_new_pam_directory.existing.notes
}

output "pam_directory_folder" {
  value = data.commander_new_pam_directory.existing.folder
}

output "pam_directory_settings" {
  description = "PAM settings object (connection, tunnel, etc.)."
  value       = data.commander_new_pam_directory.existing.pam_settings
}

# Per-user share permissions. Map key = user email; value = role string
# (viewer | share-manager | content-manager | content-share-manager | full-manager).
output "pam_directory_share" {
  description = "Per-user share permissions for this record."
  value       = data.commander_new_pam_directory.existing.share
}

# ------------------------------------------------------------------
# Example 2 - Use data source to reference a directory in another resource
# ------------------------------------------------------------------
data "commander_new_pam_directory" "corp_dc" {
  pam_directory = "_REPLACE_WITH_RECORD_UID_OR_NAME_"
}

output "corp_dc_configuration" {
  description = "PAM configuration UID linked to this directory."
  value       = data.commander_new_pam_directory.corp_dc.pam_settings.configuration
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `pam_directory` (String) PAM directory record **UID** or **name** to read.

### Read-Only

- `alternative_ips` (Set of String) **List of failover IPs** for the directory, used for Discovery. Provide one IP per entry as an array of strings.
- `directory_id` (String) **Instance ID** for AD resource in Azure and AWS hosted environments
- `directory_type` (String) **Directory type**, used for formatting of messaging. Must be one of: `active_directory`, `openldap`.
- `domain_name` (String) **Domain managed by the directory**.
- `folder` (String) Folder **UID** or path to store PAM directory record in your Keeper vault. If not provided, the record will be stored in the root path of vault.
- `hostname_or_ip` (Attributes) **Hostname or IP address** with an optional administrative port for the PAM directory. (see [below for nested schema](#nestedatt--hostname_or_ip))
- `id` (String) The PAM directory record **UID** assigned by Keeper after create.
- `notes` (String) **Notes** for this PAM directory record.
- `pam_settings` (Attributes) **PAM settings** for the record, including connection, tunnel, and administrative options. (see [below for nested schema](#nestedatt--pam_settings))
- `provider_group` (String) **Provider Group** for directories hosted in Azure.
- `provider_region` (String) **AWS region** of hosted directory.
- `share` (Map of String) Map of share permissions for this folder/record. Each map **key** is a **user email**; each **value** is one of: `viewer`, `share-manager`, `content-manager`, `content-share-manager`, `full-manager`. The folder/record **owner** is managed by Keeper and is not represented in this block.
- `title` (String) **Title** of the PAM directory record.
- `use_ssl` (Boolean) Whether to use **SSL** while connecting to the directory resource.
- `user_match` (String) **Match on OU** to filter found users during Discovery. Either match the right side of the DN or surround with slashes for a regular expression. Example: `OU=Users,DC=company,DC=com` or `/OU=Users/`

<a id="nestedatt--hostname_or_ip"></a>
### Nested Schema for `hostname_or_ip`

Read-Only:

- `administrative_port` (Number) **Administrative port number** for the PAM directory connection to connect on.
- `hostname` (String) **Address of the directory resource**.


<a id="nestedatt--pam_settings"></a>
### Nested Schema for `pam_settings`

Read-Only:

- `administrative_credentials` (String)
- `allow_supply_host` (Boolean)
- `configuration` (String)
- `connection` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection))
- `tunnel` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--tunnel))

<a id="nestedatt--pam_settings--connection"></a>
### Nested Schema for `pam_settings.connection`

Read-Only:

- `connection_port` (Number)
- `enable` (Boolean)
- `kubernetes` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--kubernetes))
- `launch_credential` (String)
- `mysql` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--mysql))
- `postgresql` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--postgresql))
- `protocol` (String)
- `rdp` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--rdp))
- `sql_server` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--sql_server))
- `ssh` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--ssh))
- `telnet` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--telnet))
- `vnc` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--vnc))

<a id="nestedatt--pam_settings--connection--kubernetes"></a>
### Nested Schema for `pam_settings.connection.kubernetes`

Read-Only:

- `allow_supply_user` (Boolean)
- `backspace` (String)
- `ca_cert` (String)
- `client_cert` (String)
- `client_key` (String)
- `color_scheme` (String)
- `command` (String)
- `container` (String)
- `font_name` (String)
- `font_size` (Number)
- `ignore_cert` (Boolean)
- `namespace` (String)
- `pod` (String)
- `read_only` (Boolean)
- `recording_include_keys` (Boolean)
- `rotate_on_termination` (Boolean)
- `scrollback` (Number)
- `session_recording` (Boolean)
- `typescript_recording` (Boolean)
- `use_ssl` (Boolean)


<a id="nestedatt--pam_settings--connection--mysql"></a>
### Nested Schema for `pam_settings.connection.mysql`

Read-Only:

- `allow_supply_user` (Boolean)
- `color_scheme` (String)
- `database` (String)
- `disable_copy` (Boolean)
- `disable_csv_export` (Boolean)
- `disable_csv_import` (Boolean)
- `disable_paste` (Boolean)
- `font_name` (String)
- `font_size` (Number)
- `read_only` (Boolean)
- `recording_include_keys` (Boolean)
- `scrollback` (Number)
- `session_recording` (Boolean)
- `typescript_recording` (Boolean)


<a id="nestedatt--pam_settings--connection--postgresql"></a>
### Nested Schema for `pam_settings.connection.postgresql`

Read-Only:

- `allow_supply_user` (Boolean)
- `color_scheme` (String)
- `database` (String)
- `disable_copy` (Boolean)
- `disable_csv_export` (Boolean)
- `disable_csv_import` (Boolean)
- `disable_paste` (Boolean)
- `font_name` (String)
- `font_size` (Number)
- `read_only` (Boolean)
- `recording_include_keys` (Boolean)
- `scrollback` (Number)
- `session_recording` (Boolean)
- `typescript_recording` (Boolean)


<a id="nestedatt--pam_settings--connection--rdp"></a>
### Nested Schema for `pam_settings.connection.rdp`

Read-Only:

- `allow_supply_user` (Boolean)
- `client_name` (String)
- `color_depth` (Number)
- `console` (Boolean)
- `console_audio` (Boolean)
- `disable_audio` (Boolean)
- `disable_auth` (Boolean)
- `disable_bitmap_caching` (Boolean)
- `disable_copy` (Boolean)
- `disable_glyph_caching` (Boolean)
- `disable_offscreen_caching` (Boolean)
- `disable_paste` (Boolean)
- `dpi` (Number)
- `enable_audio_input` (Boolean)
- `enable_desktop_composition` (Boolean)
- `enable_font_smoothing` (Boolean)
- `enable_full_window_drag` (Boolean)
- `enable_menu_animations` (Boolean)
- `enable_printing` (Boolean)
- `enable_theming` (Boolean)
- `enable_touch` (Boolean)
- `enable_wallpaper` (Boolean)
- `force_lossless` (Boolean)
- `height` (Number)
- `ignore_cert` (Boolean)
- `initial_program` (String)
- `load_balance_info` (String)
- `normalize_clipboard` (String)
- `preconnection_blob` (String)
- `preconnection_id` (String)
- `read_only` (Boolean)
- `recording_include_keys` (Boolean)
- `redirected_printer_name` (String)
- `remote_app` (String)
- `remote_app_args` (String)
- `remote_app_dir` (String)
- `resize_method` (String)
- `security` (String)
- `server_layout` (String)
- `session_recording` (Boolean)
- `sftp` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--rdp--sftp))
- `timezone` (String)
- `width` (Number)

<a id="nestedatt--pam_settings--connection--rdp--sftp"></a>
### Nested Schema for `pam_settings.connection.rdp.sftp`

Read-Only:

- `enable_sftp` (Boolean)
- `sftp_directory` (String)
- `sftp_resource_uid` (String)
- `sftp_server_alive_interval` (Number)
- `sftp_user_uid` (String)



<a id="nestedatt--pam_settings--connection--sql_server"></a>
### Nested Schema for `pam_settings.connection.sql_server`

Read-Only:

- `allow_supply_user` (Boolean)
- `color_scheme` (String)
- `database` (String)
- `disable_copy` (Boolean)
- `disable_csv_export` (Boolean)
- `disable_csv_import` (Boolean)
- `disable_paste` (Boolean)
- `font_name` (String)
- `font_size` (Number)
- `read_only` (Boolean)
- `recording_include_keys` (Boolean)
- `scrollback` (Number)
- `session_recording` (Boolean)
- `typescript_recording` (Boolean)


<a id="nestedatt--pam_settings--connection--ssh"></a>
### Nested Schema for `pam_settings.connection.ssh`

Read-Only:

- `allow_supply_user` (Boolean)
- `backspace` (String)
- `color_scheme` (String)
- `command` (String)
- `disable_copy` (Boolean)
- `disable_paste` (Boolean)
- `font_name` (String)
- `font_size` (Number)
- `host_key` (String)
- `locale` (String)
- `read_only` (Boolean)
- `recording_include_keys` (Boolean)
- `scrollback` (Number)
- `server_alive_interval` (Number)
- `session_recording` (Boolean)
- `sftp` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--ssh--sftp))
- `terminal_type` (String)
- `timezone` (String)
- `typescript_recording` (Boolean)

<a id="nestedatt--pam_settings--connection--ssh--sftp"></a>
### Nested Schema for `pam_settings.connection.ssh.sftp`

Read-Only:

- `enable_sftp` (Boolean)



<a id="nestedatt--pam_settings--connection--telnet"></a>
### Nested Schema for `pam_settings.connection.telnet`

Read-Only:

- `allow_supply_user` (Boolean)
- `backspace` (String)
- `color_scheme` (String)
- `disable_copy` (Boolean)
- `disable_paste` (Boolean)
- `font_name` (String)
- `font_size` (Number)
- `login_failure_regex` (String)
- `login_success_regex` (String)
- `password_regex` (String)
- `read_only` (Boolean)
- `recording_include_keys` (Boolean)
- `scrollback` (Number)
- `session_recording` (Boolean)
- `terminal_type` (String)
- `typescript_recording` (Boolean)
- `username_regex` (String)


<a id="nestedatt--pam_settings--connection--vnc"></a>
### Nested Schema for `pam_settings.connection.vnc`

Read-Only:

- `allow_supply_user` (Boolean)
- `audio_servername` (String)
- `clipboard_encoding` (String)
- `color_depth` (Number)
- `cursor` (String)
- `dest_host` (String)
- `dest_port` (Number)
- `disable_copy` (Boolean)
- `disable_paste` (Boolean)
- `enable_audio` (Boolean)
- `force_lossless` (Boolean)
- `read_only` (Boolean)
- `recording_include_keys` (Boolean)
- `session_recording` (Boolean)
- `sftp` (Attributes) (see [below for nested schema](#nestedatt--pam_settings--connection--vnc--sftp))
- `swap_red_blue` (Boolean)

<a id="nestedatt--pam_settings--connection--vnc--sftp"></a>
### Nested Schema for `pam_settings.connection.vnc.sftp`

Read-Only:

- `enable_sftp` (Boolean)
- `sftp_directory` (String)
- `sftp_resource_uid` (String)
- `sftp_server_alive_interval` (Number)
- `sftp_user_uid` (String)




<a id="nestedatt--pam_settings--tunnel"></a>
### Nested Schema for `pam_settings.tunnel`

Read-Only:

- `enable` (Boolean)
- `local_port` (Number)
- `re_use_port` (Boolean)
- `remote_target_port` (Number)
- `use_specified_local_port` (Boolean)
